...
begin
  ...
  case p_event_type
    -- Cancellation events - user cancelled but may still have access until expiry
    when 'CANCELLATION' then
      update public.profiles
      set 
        subscription_status = 'cancelled',
        metadata = metadata || jsonb_build_object('cancelled_at', timezone('utc', now())::text),
